...
import { trackCTAClicked, trackScrollDepth, trackExitIntent } from "@/lib/amplitude"
...
export default function TradeGuardPage() {
  useEffect(() => {
    ...
    const onExitIntent = (e: MouseEvent) => {
      if (e.clientY <= 0) {
        const pct = Math.round((window.scrollY / (document.documentElement.scrollHeight - window.innerHeight)) * 100)
        trackExitIntent({ scroll_depth_reached: pct, time_on_page_sec: Math.round((Date.now() - pageLoadTime) / 1000) })
      }
    }
        ...
        <div className="grid sm:grid-cols-2 gap-8 lg:gap-14 items-center">
          ...
          <div className={`transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-wrap">
              <Link
                href="/book-demo?source=tradeguard&cta_source=hero"
                onClick={() => trackCTAClicked({ cta_name: 'Try TradeGuard Free', location: 'Hero', product_source: 'tradeguard', cta_source: 'hero' })}
    ...
    >
      <div className={`max-w-[800px] mx-auto text-center transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link
            href="/book-demo?source=tradeguard&cta_source=bottom_cta"
            onClick={() => trackCTAClicked({ cta_name: 'Start Free Trial', location: 'CTA Section', product_source: 'tradeguard', cta_source: 'bottom_cta' })}
