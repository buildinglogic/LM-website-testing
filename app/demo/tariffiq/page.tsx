...
const PRODUCT = "tariffiq"
const PRODUCT_NAME = "TariffIQ"
const PRODUCT_COLOR = "#1B4F8A"
const VIDEO_SRC = "/videos/HSN Lookup Final Demo.mp4"
const TEASER = "See how TariffIQ classifies products to 8-digit ITC-HS and compares RoDTEP vs Drawback earnings."
const PRODUCT_HREF = "/products/tariffiq"
...
export default function TariffIQDemoPage() {
          ...
          <div className={`text-center mb-6 transition-all duration-500 delay-100 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <div className="flex items-center justify-center gap-2.5 mb-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${PRODUCT_COLOR}15` }}>
                <span className="text-[14px] font-extrabold tracking-tight" style={{ color: PRODUCT_COLOR }}>TQ</span>
              </div>
            ...
            {unlocked ? (
              ...
              <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid #E2E8F0", boxShadow: "0 8px 40px rgba(0,102,204,0.12)" }}>
                ...
                <form onSubmit={handleSubmit} className="p-6 text-center" style={{ background: "#FFFFFF" }}>
                  <h3 className="text-[15px] font-bold mb-1" style={{ color: "#0F172A" }}>Watch TariffIQ in action</h3>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 py-2.5 rounded-lg text-[13px] mb-3 focus:outline-none focus:ring-2 focus:ring-[#1B4F8A]"
