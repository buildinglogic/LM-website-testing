...
import { trackCTAClicked, trackJourneyStepViewed, trackPartnerInteracted, trackVideoPlayed, trackAwardInteracted } from "@/lib/amplitude"
          ...
          {products.map((product, idx) => {
            ...
            return (
              <Link
                key={product.name}
                href={product.href}
                onClick={() => trackCTAClicked({ cta_name: `Explore ${product.name}`, location: 'Product Showcase', product_source: 'homepage' })}
...
function HowItWorks() {
  ...
  useEffect(() => {
    if (!trackedStepsRef.current.has(activeIdx)) {
      ...
      trackJourneyStepViewed(activeIdx + 1, steps[activeIdx].title)  // fires journey_step_viewed
    }
  }, [activeIdx])
    ...
    >
      <div className="w-full max-w-[1100px] mx-auto">
        ...
        <div className={`text-center transition-all duration-700 delay-400 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <Link
            href="/book-demo?source=homepage&cta_source=how_it_works"
            onClick={() => trackCTAClicked({ cta_name: 'Watch It Happen Live', location: 'How It Works', product_source: 'homepage', cta_source: 'how_it_works' })}
    ...
    <section id="video-demo" ref={ref} className="page-snap flex flex-col justify-center py-6 lg:py-8 px-4 lg:px-8" style={{ background: "#F8FAFC" }}>
      <div className="w-full max-w-[800px] mx-auto">
        ...
        <div className={`text-center mt-4 transition-all duration-700 delay-300 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <Link
            href="/book-demo?source=homepage&cta_source=video_section"
            onClick={() => trackCTAClicked({ cta_name: 'Book a Live Demo', location: 'Video Section', product_source: 'homepage', cta_source: 'video_section' })}
